...
check("donate: step two asks for details, card and frequency", (() => { const h = read("/donate/"); return /data-donate-details hidden/.test(h) && /<form class="stack"[^>]*data-donate-form novalidate>/.test(h) && ["dname","demail","dcard","dexp","dcvc","dcardname","dzip"].every((id) => new RegExp('id="' + id + '"[^>]*required').test(h)) && count(h, /<button class="amount amount--freq" type="button" data-freq="(once|monthly)"/g) === 2 && /data-donate-back>Change the amount</.test(h); })());
